...
impl DependentSubstanceChange {
    ...
    /// ### Arguments
    /// * `sim_time` - current simulation time to evaluate the change at
    pub fn next_amount(&mut self, sim_time: SimTime) -> Option<SubstanceConcentration> {
        let cancel_time = *self.cancel_time.read().unwrap();

        // Check cancellation of the original
        if cancel_time > SimTime::from_s(0.0) && cancel_time + self.time_diff < sim_time {
            return None
        }
...
mod tests {
    ...
    use crate::{mmol_per_L, secs, SimTimeSpan};
    ...
    fn dependent_change() {
        ...
        let diff = result1.unwrap() - (amt / 2.0);
        assert!(
            diff.to_mM().abs() < 0.01,
            "time: {}, result: {}, diff: {}",
            sim_time.to_s(),
            result1.unwrap().to_mM(),
            diff.to_mM()
        );
        ...
        let total_result = result2.unwrap() + result1.unwrap();
        ...
        sim_time = secs!(1.1);
        assert!(dep_change.next_amount(sim_time).is_none());
