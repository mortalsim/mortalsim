...
impl SubstanceChange {
    ...
    pub fn next_amount(&mut self, sim_time: Time) -> MolarConcentration {
        let new_val = MolarConcentration::new::<mole_per_cubic_meter>(
            self.bound_fn.call((sim_time - self.start_time).value, self.duration.value, self.amount.value)
        );


        let result = new_val - self.previous_val;
        println!("new_val: {:?}, previous_val: {:?}, result: {:?}", new_val, self.previous_val, result);
...
mod tests {
    ...
    use super::{SubstanceChange, Time, MolarConcentration, BoundFn};
    ...
    fn change_call() {
        ...
        assert!(diff.value.abs() < 0.01, "time: {}, result: {}, diff: {}", sim_time.value, result.value, diff.value);
