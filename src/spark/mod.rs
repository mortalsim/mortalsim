...
impl<T: ModuleA> ModuleWrapper for ModuleAWrapper<T> {
  fn has_a(&self) -> bool {true}
  fn has_b(&self) -> bool {false}
}
...
impl<T: ModuleB> ModuleWrapper for ModuleBWrapper<T> {
  fn has_a(&self) -> bool {false}
  fn has_b(&self) -> bool {true}
}
...
impl MainObject {
  fn new() -> MainObject {
    MainObject {
      component_a: ComponentA {},
      component_b: ComponentB {},
      keeper: ModuleKeeper { modules: Vec::new() },
    } 
  }
...
mod tests {
  ...
  #[test]
  fn basic_test() {
    let a = ModuleSoleA { connector_a: ConnectorA {  }};
    let b = ModuleSoleB { connector_b: ConnectorB {  }};
    let ab = ModuleAB { connector_a: ConnectorA {  }, connector_b: ConnectorB {  }};
