...
pub struct ClosedCirculationManager<V: BloodVessel + 'static> {
    manager_id: Uuid,
...
impl<V: BloodVessel + 'static> ClosedCirculationManager<V> {
    ...
    pub fn new(system: ClosedCirculatorySystem<V>) -> ClosedCirculationManager<V> {
    ...
    fn get_system(&self) -> &ClosedCirculatorySystem<V> {
        match &self.system {
            None => panic!("ClosedCirculatorySystem was not reclaimed for ClosedCirculationManager!"),
            Some(v) => v
        }
    }
