...
/// Error when an ID has already been returned to the generator
...
/// Generator for unique, reusable identifiers
...
impl IdGenerator {
    ...
    pub fn return_id(&mut self,  id: IdType) -> Result<(), DuplicateIdReturnError> {
        if self.available_ids.iter().any(|&i| i == id) {
            // return an error when an id which was already returned is returned again
            return Err(DuplicateIdReturnError {generator_id: self.generator_id, dup_id: id});
        }
...
mod tests {
    ...
    fn dup_return() {
        ...
        assert_eq!(res.unwrap_err().dup_id, id1);
    }
}